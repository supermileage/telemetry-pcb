EESchema Schematic File Version 4
EELAYER 30 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 1 4
Title ""
Date ""
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Sheet
S 5300 800  1450 1050
U 6188C66E
F0 "Power & CAN Distribution" 50
F1 "power_can.sch" 50
F2 "12V" O R 6750 1600 50 
F3 "CANH" B R 6750 1050 50 
F4 "CANL" B R 6750 1250 50 
F5 "VSENS" O L 5300 1600 50 
$EndSheet
Wire Wire Line
	2300 1450 2500 1450
Wire Wire Line
	2300 1550 2500 1550
Wire Wire Line
	1250 1650 1450 1650
Wire Wire Line
	2500 1650 2300 1650
Wire Wire Line
	2500 1750 2300 1750
Wire Wire Line
	2500 1850 2300 1850
Wire Wire Line
	2500 1950 2300 1950
Wire Wire Line
	2500 2050 2300 2050
Wire Wire Line
	2500 2150 2300 2150
Wire Wire Line
	2500 2250 2300 2250
Text Label 2350 1950 0    50   ~ 0
MO1
Text Label 2350 2050 0    50   ~ 0
SCK1
$Comp
L Device:R R5
U 1 1 614C1C41
P 3150 3600
F 0 "R5" H 3220 3646 50  0000 L CNN
F 1 "68" H 3220 3555 50  0000 L CNN
F 2 "Resistor_SMD:R_0805_2012Metric" V 3080 3600 50  0001 C CNN
F 3 "~" H 3150 3600 50  0001 C CNN
	1    3150 3600
	0    -1   -1   0   
$EndComp
$Comp
L Device:R R4
U 1 1 614AF40B
P 7500 4050
F 0 "R4" H 7300 4100 50  0000 L CNN
F 1 "5.6" H 7300 4000 50  0000 L CNN
F 2 "Resistor_SMD:R_0805_2012Metric" V 7430 4050 50  0001 C CNN
F 3 "~" H 7500 4050 50  0001 C CNN
	1    7500 4050
	0    -1   -1   0   
$EndComp
$Comp
L Device:R R3
U 1 1 61485C57
P 8400 3900
F 0 "R3" H 8470 3946 50  0000 L CNN
F 1 "5.6" H 8470 3855 50  0000 L CNN
F 2 "Resistor_SMD:R_0805_2012Metric" V 8330 3900 50  0001 C CNN
F 3 "~" H 8400 3900 50  0001 C CNN
	1    8400 3900
	0    -1   -1   0   
$EndComp
$Comp
L Device:LED L4
U 1 1 61472AFA
P 2700 3600
F 0 "L4" V 2750 3800 50  0000 R CNN
F 1 "ORANGE" V 2650 4000 50  0000 R CNN
F 2 "LED_SMD:LED_0805_2012Metric" H 2700 3600 50  0001 C CNN
F 3 "~" H 2700 3600 50  0001 C CNN
	1    2700 3600
	1    0    0    -1  
$EndComp
$Comp
L Device:LED L3
U 1 1 614602B7
P 7950 4050
F 0 "L3" V 8000 4200 50  0000 R CNN
F 1 "BLUE" V 7900 4300 50  0000 R CNN
F 2 "LED_SMD:LED_0805_2012Metric" H 7950 4050 50  0001 C CNN
F 3 "~" H 7950 4050 50  0001 C CNN
	1    7950 4050
	-1   0    0    1   
$EndComp
Wire Wire Line
	1450 1150 950  1150
Wire Wire Line
	950  950  1450 950 
$Comp
L Connector_Generic:Conn_01x15 T4
U 1 1 603BA196
P 1650 1550
F 0 "T4" H 1730 1592 50  0000 L CNN
F 1 "GPIO_L" H 1730 1501 50  0000 L CNN
F 2 "Connector_PinSocket_2.54mm:PinSocket_1x15_P2.54mm_Vertical" H 1650 1550 50  0001 C CNN
F 3 "~" H 1650 1550 50  0001 C CNN
	1    1650 1550
	1    0    0    -1  
$EndComp
$Comp
L Device:LED L2
U 1 1 61386F02
P 8850 3900
F 0 "L2" V 8889 3782 50  0000 R CNN
F 1 "GREEN" V 8798 3782 50  0000 R CNN
F 2 "LED_SMD:LED_0805_2012Metric" H 8850 3900 50  0001 C CNN
F 3 "~" H 8850 3900 50  0001 C CNN
	1    8850 3900
	-1   0    0    1   
$EndComp
$Comp
L power:+3V3 #PWR0116
U 1 1 605721ED
P 950 750
F 0 "#PWR0116" H 950 600 50  0001 C CNN
F 1 "+3V3" H 965 923 50  0000 C CNN
F 2 "" H 950 750 50  0001 C CNN
F 3 "" H 950 750 50  0001 C CNN
	1    950  750 
	1    0    0    -1  
$EndComp
Text Label 1250 2250 0    50   ~ 0
TX
Text Label 1250 2150 0    50   ~ 0
RX
Text Label 1250 2050 0    50   ~ 0
MI
Text Label 1250 1950 0    50   ~ 0
MO
Text Label 1250 1750 0    50   ~ 0
A5
Text Label 1250 1650 0    50   ~ 0
A4
Text Label 1250 1550 0    50   ~ 0
A3
Text Label 1250 1450 0    50   ~ 0
A2
Text Label 1250 1350 0    50   ~ 0
A1
Text Label 1250 1250 0    50   ~ 0
A0
Text Label 1250 1050 0    50   ~ 0
MD
Text Label 1250 850  0    50   ~ 0
RST
Wire Wire Line
	1450 850  1250 850 
Wire Wire Line
	1450 1050 1250 1050
Wire Wire Line
	1450 1250 1250 1250
Wire Wire Line
	1450 1350 1250 1350
Wire Wire Line
	1450 1450 1250 1450
Wire Wire Line
	1450 1550 1250 1550
Wire Wire Line
	1450 1750 1250 1750
Wire Wire Line
	1450 1850 1250 1850
Wire Wire Line
	1450 1950 1250 1950
Wire Wire Line
	1450 2050 1250 2050
Wire Wire Line
	1450 2150 1250 2150
Wire Wire Line
	1450 2250 1250 2250
Text Label 1250 1850 0    50   ~ 0
SCK
Text Label 2350 2250 0    50   ~ 0
SDA
Text Label 2350 2150 0    50   ~ 0
SCL
Text Label 2350 1850 0    50   ~ 0
MI1
Text Label 2350 1750 0    50   ~ 0
D5
Text Label 2350 1650 0    50   ~ 0
D6
Text Label 2350 1550 0    50   ~ 0
D7
Text Label 2350 1450 0    50   ~ 0
D8
Text Label 2350 1350 0    50   ~ 0
VIN
Text Label 2350 1250 0    50   ~ 0
EN
Text Label 2350 1150 0    50   ~ 0
LI+
Wire Wire Line
	2500 1350 2300 1350
Wire Wire Line
	2500 1250 2300 1250
Wire Wire Line
	2500 1150 2300 1150
$Comp
L Connector_Generic:Conn_01x12 T5
U 1 1 603BCCF9
P 2100 1650
F 0 "T5" H 2018 2367 50  0000 C CNN
F 1 "GPIO_R" H 2018 2276 50  0000 C CNN
F 2 "Connector_PinSocket_2.54mm:PinSocket_1x12_P2.54mm_Vertical" H 2100 1650 50  0001 C CNN
F 3 "~" H 2100 1650 50  0001 C CNN
	1    2100 1650
	-1   0    0    -1  
$EndComp
$Comp
L power:GND #PWR0106
U 1 1 6051971B
P 5250 6250
F 0 "#PWR0106" H 5250 6000 50  0001 C CNN
F 1 "GND" H 5255 6077 50  0000 C CNN
F 2 "" H 5250 6250 50  0001 C CNN
F 3 "" H 5250 6250 50  0001 C CNN
	1    5250 6250
	1    0    0    -1  
$EndComp
$Comp
L power:PWR_FLAG #FLG0103
U 1 1 60673E4C
P 5250 2700
F 0 "#FLG0103" H 5250 2775 50  0001 C CNN
F 1 "PWR_FLAG" V 5250 2827 50  0000 L CNN
F 2 "" H 5250 2700 50  0001 C CNN
F 3 "~" H 5250 2700 50  0001 C CNN
	1    5250 2700
	0    -1   -1   0   
$EndComp
$Comp
L power:+3V3 #PWR0105
U 1 1 60459E30
P 5250 2600
F 0 "#PWR0105" H 5250 2450 50  0001 C CNN
F 1 "+3V3" H 5265 2773 50  0000 C CNN
F 2 "" H 5250 2600 50  0001 C CNN
F 3 "" H 5250 2600 50  0001 C CNN
	1    5250 2600
	1    0    0    -1  
$EndComp
Text Label 3800 4950 0    50   ~ 0
RX
Text Label 3800 5100 0    50   ~ 0
TX
Text Label 3800 4800 0    50   ~ 0
MI
Text Label 3800 4650 0    50   ~ 0
MO
Text Label 3800 4500 0    50   ~ 0
SCK
Text Label 3800 4350 0    50   ~ 0
A5
Text Label 3800 4200 0    50   ~ 0
A4
Text Label 3800 4050 0    50   ~ 0
A3
Text Label 3800 3900 0    50   ~ 0
A2
Text Label 3800 3750 0    50   ~ 0
A1
Text Label 3800 3600 0    50   ~ 0
A0
Text Label 3800 3450 0    50   ~ 0
MD
Text Label 3800 3300 0    50   ~ 0
RST
Wire Wire Line
	6950 3450 7200 3450
Wire Wire Line
	6950 3750 7200 3750
Wire Wire Line
	6950 4350 7200 4350
Wire Wire Line
	6950 4500 7200 4500
Wire Wire Line
	6950 4650 7200 4650
Wire Wire Line
	6950 4800 7200 4800
Text Label 7000 4800 0    50   ~ 0
SCK1
Text Label 7000 4650 0    50   ~ 0
MO1
Text Label 7000 4500 0    50   ~ 0
MI1
Text Label 7000 5100 0    50   ~ 0
SDA
Text Label 7000 4950 0    50   ~ 0
SCL
Text Label 7000 4350 0    50   ~ 0
D5
Text Label 7000 4200 0    50   ~ 0
D6
Text Label 7000 4050 0    50   ~ 0
D7
Text Label 7000 3900 0    50   ~ 0
D8
Text Label 7000 3750 0    50   ~ 0
EN
Text Label 7000 3450 0    50   ~ 0
LI+
Text Label 7000 3600 0    50   ~ 0
VIN
Wire Wire Line
	5250 2600 5250 2700
Wire Wire Line
	6950 4950 7350 4950
Connection ~ 5250 2700
Wire Wire Line
	5250 2700 5250 2850
Wire Wire Line
	6950 4200 7200 4200
$Comp
L Connector:Conn_01x02_Female S3
U 1 1 616E1EEC
P 2350 4000
F 0 "S3" H 2378 3976 50  0000 L CNN
F 1 "LOG_BTN" H 2378 3885 50  0000 L CNN
F 2 "TestPoint:TestPoint_2Pads_Pitch2.54mm_Drill0.8mm" H 2350 4000 50  0001 C CNN
F 3 "~" H 2350 4000 50  0001 C CNN
	1    2350 4000
	-1   0    0    1   
$EndComp
Wire Wire Line
	7350 5200 7350 4950
$Sheet
S 800  6000 1500 950 
U 61951597
F0 "Serial Peripherals" 50
F1 "serial.sch" 50
F2 "RX" O R 2300 6300 50 
F3 "TX" O R 2300 6500 50 
$EndSheet
$Sheet
S 9200 1150 1400 1000
U 6195E29D
F0 "SPI Peripherals" 50
F1 "spi.sch" 50
F2 "CANH" O L 9200 1300 50 
F3 "CANL" O L 9200 1400 50 
F4 "MO1" O L 9200 1600 50 
F5 "MI1" O L 9200 1700 50 
F6 "CAN_CS" O L 9200 1900 50 
F7 "SCK1" O L 9200 1800 50 
F8 "CAN_INT" O L 9200 2000 50 
F9 "SCK" O R 10600 1500 50 
F10 "MO" O R 10600 1300 50 
F11 "MI" O R 10600 1400 50 
F12 "IMU_CS" O R 10600 1600 50 
F13 "T1_CS" O R 10600 1700 50 
F14 "T2_CS" O R 10600 1800 50 
$EndSheet
$Comp
L Connector_Generic:Conn_01x08 H3
U 1 1 61991C85
P 8600 4800
AR Path="/61991C85" Ref="H3"  Part="1" 
AR Path="/61951597/61991C85" Ref="H?"  Part="1" 
F 0 "H3" H 8680 4792 50  0000 L CNN
F 1 "GPS" H 8680 4701 50  0000 L CNN
F 2 "Connector_PinSocket_2.54mm:PinSocket_1x08_P2.54mm_Vertical" H 8600 4800 50  0001 C CNN
F 3 "~" H 8600 4800 50  0001 C CNN
	1    8600 4800
	1    0    0    -1  
$EndComp
$Comp
L power:+3V3 #PWR0101
U 1 1 61991C8D
P 7700 4650
AR Path="/61991C8D" Ref="#PWR0101"  Part="1" 
AR Path="/61951597/61991C8D" Ref="#PWR?"  Part="1" 
F 0 "#PWR0101" H 7700 4500 50  0001 C CNN
F 1 "+3V3" H 7715 4823 50  0000 C CNN
F 2 "" H 7700 4650 50  0001 C CNN
F 3 "" H 7700 4650 50  0001 C CNN
	1    7700 4650
	1    0    0    -1  
$EndComp
NoConn ~ 8400 4500
NoConn ~ 8400 4600
NoConn ~ 8400 4700
NoConn ~ 8400 4800
Wire Wire Line
	6950 5100 8400 5100
Wire Wire Line
	8400 5200 7350 5200
Wire Wire Line
	7600 3600 6950 3600
Wire Wire Line
	6750 1600 7600 1600
Wire Wire Line
	7600 1600 7600 3600
$Comp
L power:GND #PWR0102
U 1 1 61BB9166
P 950 1300
F 0 "#PWR0102" H 950 1050 50  0001 C CNN
F 1 "GND" H 955 1127 50  0000 C CNN
F 2 "" H 950 1300 50  0001 C CNN
F 3 "" H 950 1300 50  0001 C CNN
	1    950  1300
	1    0    0    -1  
$EndComp
Wire Wire Line
	950  1300 950  1150
Wire Wire Line
	950  750  950  950 
Wire Wire Line
	6950 4050 7350 4050
Wire Wire Line
	7650 4050 7800 4050
Wire Wire Line
	6950 3900 8250 3900
Wire Wire Line
	8550 3900 8700 3900
Wire Wire Line
	7700 5000 8400 5000
Wire Wire Line
	7700 4650 7700 5000
Wire Wire Line
	8400 4900 8150 4900
Wire Wire Line
	8150 4900 8150 5400
$Comp
L power:GND #PWR0103
U 1 1 61BE6726
P 8150 5400
F 0 "#PWR0103" H 8150 5150 50  0001 C CNN
F 1 "GND" H 8155 5227 50  0000 C CNN
F 2 "" H 8150 5400 50  0001 C CNN
F 3 "" H 8150 5400 50  0001 C CNN
	1    8150 5400
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0104
U 1 1 61BE6C3E
P 9250 4150
F 0 "#PWR0104" H 9250 3900 50  0001 C CNN
F 1 "GND" H 9255 3977 50  0000 C CNN
F 2 "" H 9250 4150 50  0001 C CNN
F 3 "" H 9250 4150 50  0001 C CNN
	1    9250 4150
	1    0    0    -1  
$EndComp
Wire Wire Line
	8100 4050 9250 4050
Wire Wire Line
	9250 4050 9250 4150
Wire Wire Line
	9000 3900 9250 3900
Wire Wire Line
	9250 3900 9250 4050
Connection ~ 9250 4050
Wire Wire Line
	3000 3600 2850 3600
$Comp
L power:GND #PWR0107
U 1 1 61C12391
P 3250 4400
F 0 "#PWR0107" H 3250 4150 50  0001 C CNN
F 1 "GND" H 3255 4227 50  0000 C CNN
F 2 "" H 3250 4400 50  0001 C CNN
F 3 "" H 3250 4400 50  0001 C CNN
	1    3250 4400
	1    0    0    -1  
$EndComp
Wire Wire Line
	3550 3750 3550 1600
Wire Wire Line
	3550 1600 5300 1600
Wire Wire Line
	2550 4000 3250 4000
Wire Wire Line
	2550 3600 1550 3600
$Comp
L power:GND #PWR0108
U 1 1 61C28BF5
P 1550 4400
F 0 "#PWR0108" H 1550 4150 50  0001 C CNN
F 1 "GND" H 1555 4227 50  0000 C CNN
F 2 "" H 1550 4400 50  0001 C CNN
F 3 "" H 1550 4400 50  0001 C CNN
	1    1550 4400
	1    0    0    -1  
$EndComp
Wire Wire Line
	1550 3600 1550 4400
Wire Wire Line
	2300 6300 3000 6300
Wire Wire Line
	3000 6300 3000 4950
Wire Wire Line
	2300 6500 3200 6500
Wire Wire Line
	3200 6500 3200 5100
Wire Wire Line
	3200 5100 4000 5100
Wire Wire Line
	3000 4950 4000 4950
Wire Wire Line
	4000 4800 3800 4800
Wire Wire Line
	4000 4650 3800 4650
Wire Wire Line
	4000 4500 3800 4500
Wire Wire Line
	4000 4350 3800 4350
Wire Wire Line
	4000 4200 3800 4200
Wire Wire Line
	2550 3900 4000 3900
Wire Wire Line
	3550 3750 4000 3750
Wire Wire Line
	3300 3600 4000 3600
Wire Wire Line
	4000 3450 3800 3450
Wire Wire Line
	4000 3300 3800 3300
$Comp
L telemetry-pcb-rescue:Particle_Boron-particle-telemetry-pcb-rescue H1,H2
U 1 1 602A0E66
P 5450 4050
F 0 "H1,H2" H 5600 2750 50  0000 C CNN
F 1 "Particle_Boron" H 5500 4900 50  0000 C CNN
F 2 "telemetry-pcb:Particle_boron" H 4750 4250 50  0001 C CNN
F 3 "" H 4750 4250 50  0001 C CNN
	1    5450 4050
	1    0    0    -1  
$EndComp
$Comp
L power:+3V3 #PWR0109
U 1 1 61CC4990
P 6200 5800
F 0 "#PWR0109" H 6200 5650 50  0001 C CNN
F 1 "+3V3" H 6215 5973 50  0000 C CNN
F 2 "" H 6200 5800 50  0001 C CNN
F 3 "" H 6200 5800 50  0001 C CNN
	1    6200 5800
	1    0    0    -1  
$EndComp
Wire Wire Line
	5250 5500 5250 6050
Wire Wire Line
	5250 6050 6400 6050
Connection ~ 5250 6050
Wire Wire Line
	5250 6050 5250 6250
Wire Wire Line
	6400 5950 6200 5950
Wire Wire Line
	6200 5950 6200 5800
Wire Wire Line
	9200 1300 8800 1300
Wire Wire Line
	8800 1300 8800 1050
Wire Wire Line
	8800 1050 6750 1050
Wire Wire Line
	6750 1250 8600 1250
Wire Wire Line
	8600 1250 8600 1400
Wire Wire Line
	8600 1400 9200 1400
Wire Wire Line
	9200 1600 8900 1600
Wire Wire Line
	8900 1700 9200 1700
Wire Wire Line
	8900 1800 9200 1800
Wire Wire Line
	8900 1900 9200 1900
Wire Wire Line
	8900 2000 9200 2000
Text Label 8900 1600 0    50   ~ 0
MO1
Text Label 8900 1700 0    50   ~ 0
MI1
Text Label 8900 1800 0    50   ~ 0
SCK1
Text Label 8900 1900 0    50   ~ 0
D5
Text Label 8900 2000 0    50   ~ 0
D6
Wire Wire Line
	10600 1300 10900 1300
Wire Wire Line
	10600 1400 10900 1400
Wire Wire Line
	10900 1500 10600 1500
Wire Wire Line
	10900 1600 10600 1600
Wire Wire Line
	10900 1700 10600 1700
Wire Wire Line
	10900 1800 10600 1800
Text Label 10750 1300 0    50   ~ 0
SCK
Text Label 10800 1400 0    50   ~ 0
MI
Text Label 10800 1500 0    50   ~ 0
MO
Text Label 10800 1600 0    50   ~ 0
A3
Text Label 10800 1700 0    50   ~ 0
A5
Text Label 10800 1800 0    50   ~ 0
A4
Wire Wire Line
	3250 4000 3250 4400
Wire Wire Line
	3800 4050 4000 4050
$Comp
L Connector:Conn_01x02_Female T1
U 1 1 61CAF3A3
P 6600 5950
AR Path="/61CAF3A3" Ref="T1"  Part="1" 
AR Path="/6195E29D/61CAF3A3" Ref="T?"  Part="1" 
F 0 "T1" V 6446 5762 50  0000 R CNN
F 1 "TEST_3V3" V 6537 5762 50  0000 R CNN
F 2 "TestPoint:TestPoint_2Pads_Pitch2.54mm_Drill0.8mm" H 6600 5950 50  0001 C CNN
F 3 "~" H 6600 5950 50  0001 C CNN
	1    6600 5950
	1    0    0    -1  
$EndComp
$EndSCHEMATC
